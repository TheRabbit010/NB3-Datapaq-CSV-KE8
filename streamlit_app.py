...
# 5. ฟังก์ชันอ่านไฟล์ CSV และดึงข้อมูลบริสุทธิ์ของไฟล์ที่อัปโหลด
...
# 📌 ปรับให้รับเพียง 1 ไฟล์เท่านั้น
...
if uploaded_file:
  ...
  if df.empty:
    ...
    # 📌 ตรวจจับสูตรเพื่อกำหนดช่วงเวลาประมวลผลโซนอัตโนมัติ
    raw_text_meta = (
        metadata.get("raw_text", "") + " " + metadata.get("title", "")
    ).upper()
    ...
    for p_num, col_name in ordered_cols:
      ...
      # Max Temp
      br_max = (
          f"{brazing_max_subset[col_name].max():.1f}"
          if not brazing_max_subset.empty
          else "0.0"
      )
      d_max = (
          f"{dryer_subset[col_name].max():.1f}"
